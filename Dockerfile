...
RUN apk update \
	&& apk upgrade \
	&& apk add json-c openssl ipset iptables libnl3 libnfnetlink libevent \
	...
	&& curl https://download.strongswan.org/strongswan-5.7.2.tar.gz | tar xz --strip-components=1 -C . \
	&& autoreconf -i -f \
	\
